...
const RepoActionSection: React.FC<{ owner: string; repo: string }> = ({ owner, repo }) => {
  ...
  const { mutate: createPRs } = api.github.createPRs.useMutation();
  const repoData = api.github.getRepoData.useQuery({ owner, repo });
  ...
  const compareBranchesResponse = api.github.compareBranches.useQuery(
